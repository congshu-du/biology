...
const AsSankey = defineComponent(
  (props) => {
    ...
    const Item = styled.div<{ checked: boolean }>`
      ...
      background-color: ${(p) => (p.checked ? token.value.colorPrimaryBg : "")};
      padding: 0 8px;
    `;

    return () => (
      <Wrapper
        style={{ height: "calc(100vh - 170px)", minHeight: "800px" }}
        class="h-full w-full flex flex-col overflow-hidden"
      >
        <div class="flex-1  pt-0 flex">
          <LeftContainer class="w-[300px]">
            <Item class="text-base font-semibold">
              目标AS:{" "}
              <Select
                value={target.value}
