...
const ProviderPie = defineComponent<{ type: "IPv4" | "IPv6"; list: any[] }>(
  (props) => {
    const option = computed(() => {
      const { type, list } = props;
      const data = list.filter((item) => item.category === "provider");
      data.sort((a, b) => b.degree - a.degree);
      const lastData = data.slice(0, 5);
      return {
        title: {
          text: `${type} Provider排行榜`,
          left: "center",
          top: "8px",
        },
        tooltip: {
