      ...
      return {
        ...
        tooltip: {
          ...
          formatter: function (params: any) {
            // params 为不同 series 的数组，按顺序：延迟, 速度, 丢包
            const idx = params[0].dataIndex;
            const p = list.value[idx];
            return `
              <div style="padding:8px; min-width:200px;">
                <div style="font-weight:700; margin-bottom:6px;">${p?.name}</div>
                <div style="font-size:12px; color:#444;">网络延迟: <strong style="color:${colorList[0]};">${p?.latency ?? "-"} ms</strong></div>
                <div style="font-size:12px; color:#444;">下载速度: <strong style="color:${colorList[1]};">${p?.speed ?? "-"} MB/s</strong></div>
                <div style="font-size:12px; color:#444;">丢包率: <strong style="color:${colorList[2]};">${p?.packetLoss != null ? (p.packetLoss * 100).toFixed(2) + "%" : "-"}</strong></div>
              </div>
            `;
          },
        },
        ...
        legend: { top: 36, data: ["网络延迟", "下载速度", "丢包率"] },
        series: [
          {
            name: "网络延迟",
            type: "bar",
            data: nl.map((v, i) => ({ value: v, raw: latencies[i], itemStyle: { color: colorList[0] } })),
            barGap: 0,
          ...
          },
          {
            name: "下载速度",
            type: "bar",
            data: ns.map((v, i) => ({ value: v, raw: speeds[i], itemStyle: { color: colorList[1] } })),
            label: { show: true, position: "top", formatter: (p: any) => `${p.data.raw} MB/s`, fontSize: 11 },
          },
          {
            name: "丢包率",
            type: "bar",
            data: np.map((v, i) => ({ value: v, raw: losses[i], itemStyle: { color: colorList[2] } })),
