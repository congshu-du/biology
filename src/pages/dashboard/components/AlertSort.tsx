        ...
        tooltip: {
          ...
          formatter: function (params: any) {
            ...
            <div style="padding:8px; min-width:200px;">
              ...
              <div style="font-size:12px; color:#444;">网络延迟: <strong style="color:${colorList[0]};">${latency.value[idx]} ms</strong></div>
              <div style="font-size:12px; color:#444;">下载速度: <strong style="color:${colorList[1]};">${speed.value[idx]} MB/s</strong></div>
              <div style="font-size:12px; color:#444;">丢包率: <strong style="color:${colorList[2]};">${loss.value[idx]}%</strong></div>
            </div>
          `;
          },
        },
        ...
        series: [
          {
            name: "网络延迟",
            type: "bar",
            data: nl.map((v, i) => ({ value: v, raw: latency.value[i], itemStyle: { color: colorList[0] } })),
            label: { show: true, position: "top", formatter: (p: any) => `${p.data.raw} ms` },
          },
          {
            name: "下载速度",
            type: "bar",
            data: ns.map((v, i) => ({ value: v, raw: speed.value[i], itemStyle: { color: colorList[1] } })),
            label: { show: true, position: "top", formatter: (p: any) => `${p.data.raw} MB/s` },
          },
          {
            name: "丢包率",
            type: "bar",
            data: np.map((v, i) => ({ value: v, raw: loss.value[i], itemStyle: { color: colorList[2] } })),
            label: { show: true, position: "top", formatter: (p: any) => `${p.data.raw}%` },
          },
        ],
        graphic: {
          invisible: latency.value.length !== 0,
          type: "text",
          left: "center",
          top: "middle",
